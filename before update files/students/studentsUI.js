...
export async function showStudentsTab(studentTabManager) {
    ...
    if (studentTabManager) {
        studentTabManager.switchTab(elements.activeStudentsTabButton);
    } else {
    ...
    try {
        await filterAndRenderStudents({ columns: baseColumns, rowClickHandler: handleActiveStudentsClick });
    } catch (error) {
...
export function handleActiveStudentsClick(student) {
...
const jpegPhotoPath = `http://localhost:3000/student-data/${fullYear}/${student.studentId}/photo.jpeg?t=${Date.now()}`;
    ...
    const addActionBtn = (handler, iconClass, label) => {
        ...
        const btn = createActionButton({
            ...
            actionHandler: (selectedIds) => {
                handler(selectedIds);
                filterAndRenderStudents(); // Refresh after action
            },
        });
